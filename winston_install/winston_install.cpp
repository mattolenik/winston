...
#include "winston.gob.bin.h"
#include "elevate.exe.bin.h"
#include "elevate.dll.bin.h"
#include "Prereqs.h"
...
int __cdecl wmain(int argc, wchar_t *argv[], wchar_t *envp[])
{
    ...
    auto gob = winston_gob;
    auto data = reinterpret_cast<const char*>(gob);

    size_t c = 0;
    while (c < winston_gob_length)
    {
        byte type = data[c];
        c += sizeof(byte);
        std::wstring name(reinterpret_cast<const wchar_t*>(&data[c]));
        c += 128 * sizeof(wchar_t);
        UINT64 size = *reinterpret_cast<const UINT64*>(&data[c]);
        c += sizeof(UINT64);

        std::wstring path = installSource.Path() + L"\\"s + name;
